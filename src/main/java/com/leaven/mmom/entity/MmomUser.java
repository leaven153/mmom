 ...
 *        createdAt - 가입일 (BaseEntity 쓰지 않는다. ∵ updatedAt을 쓰지 않고 그냥) lastConnected,
 *        mypage_sortType - 프로젝트 모아보기에서 사용자가 기본으로 선택한 정렬(최종수정일순, 생성일순, 마감일순),
 *        user payment info - 유료 구독일 경우에만 사용
...
public class MmomUser {
    ...
    @Column
    private LocalDateTime visitedAt; // 로그인할 때마다 now를 넣어줘야되겠네?!
    ...
    public void setLastConnected(LocalDateTime visitedAt) {
